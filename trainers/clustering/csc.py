...
from models import CSCGCN


class CSCTrainer(BaseTrainer):
    """
    ...
    def train(self, model, train_loader, optimizer, criterion=None, scheduler=None, **kwargs):
        loss_record = LossRecord(["train_loss", "mc_loss", "o_loss", "contrastive_loss"])
        model.cuda()
        model.train()
        
        if self.dataset == "PubMed":
            train_loader = NeighborLoader(train_loader, batch_size=64, shuffle=True, num_neighbors=[10, 10])
        ...
        for graph in train_loader:
            ...
            edge_weight = graph.edge_weight.to(self.device)
            edge_index = graph.edge_index.to(self.device)
            
            loss, mc_loss, o_loss, contrastive_loss = model.loss(x, edge_index, edge_weight, sample_size=self.sample_size)
    ...
    def evaluate(self, model, eval_loader, criterion=None, split="valid", **kwargs):
        ...
        with torch.no_grad():
            ...
            out = model.get_cluster_assignments(x, edge_index, edge_weight)
            nmi = normalized_mutual_info_score(out.cpu(), y.cpu())
            cs = completeness_score(out.cpu(), y.cpu())
